...
def main(config):
    ...
    new_colors = ["#000000","#525252","#969696","#d9d9d9","#ffffff"]
    progress_labels = sdr_level_progress + ["No data"]
    infra_sectors = ["Energy","Transport","Water"]
    ...
    infra_typology_df["UDHR_type"] = infra_typology_df["UDHR_type"].fillna("Other")
    ...
    ej_df = pd.merge(ej_df,infra_typology_df[["type","UDHR_type"]],how="left",on=["type"])
    ej_df["UDHR_type"] = ej_df["UDHR_type"].fillna("Unkown")        
    ...
    for idx,(infra,color) in enumerate(zip(infra_sectors,infra_colors)):
        nodes = ej_df[ej_df["UDHR_type"] == infra]
